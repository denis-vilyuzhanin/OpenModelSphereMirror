...
public final class BESynchronizationTool extends BEUseCaseTool {

    private Cursor cursor;

    public BESynchronizationTool(String text) {
        super(text, GraphicUtil.loadImage(DbSMSStereotype.class,
    ...
    protected Cursor loadDefaultCursor() {
        return AwtUtil.loadCursor(DbSMSStereotype.class, "resources/horiz-synchro-small.gif",
                new Point(8, 8), "synchro", true); // NOT LOCALIZABLE, not yet
    }

    public String getText(DbObject notation) {
        return LocaleMgr.action.getString("Synchronization");
    }
                ...
                while (enum2.hasMoreElements()) {
                    ...
                    if (stereotype.getStereotypeMetaClass()
                        ...
                        if (stereotype.isBuiltIn())
                            if (stereotype.getName().equals("horizontal synchro")) {
                                useCase.setUmlStereotype(stereotype);
                                useCase.setName("Synchronization");
                            }
                    }
                }
                enum2.close();
