...
public final class BEDecisionTool extends BEUseCaseTool {

    private Cursor cursor;

    public BEDecisionTool(String text) {
        super(text, GraphicUtil.loadImage(DbSMSStereotype.class, "resources/decision-small.gif"));
    ...
    protected Cursor loadDefaultCursor() {
        return AwtUtil.loadCursor(DbSMSStereotype.class, "resources/decision-small.gif", new Point(
                8, 8), "decision", true); // NOT LOCALIZABLE, not yet
    }

    public String getText(DbObject notation) {
        return LocaleMgr.action.getString("DecisionCreation");
    }
                ...
                while (enum2.hasMoreElements()) {
                    ...
                    if (stereotype.getStereotypeMetaClass()
                        ...
                        if (stereotype.isBuiltIn())
                            if (stereotype.getName().equals("decision")) {
                                useCase.setUmlStereotype(stereotype);
                                useCase.setName("Decision");
                            }
                    }
                }
                enum2.close();
