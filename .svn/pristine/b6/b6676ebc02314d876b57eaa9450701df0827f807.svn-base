...
public class PropertiesRenderer implements TableCellRenderer {
    ...
    public final Component getTableCellRendererComponent(JTable table, Object value,
        ...
        // later, BooleanRenderer, ...
        Renderer delegate;
        Object cellValue = table.getValueAt(row, column);

        if (cellValue instanceof Boolean)
            delegate = BooleanRenderer.singleton;
        else if (cellValue instanceof Color)
            delegate = ColorRenderer.singleton;
        else
            delegate = DefaultRenderer.singleton;

        if (delegate != null) {
            return delegate.getTableCellRendererComponent(table, value, isSelected, hasFocus, row,
