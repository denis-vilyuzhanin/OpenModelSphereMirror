...
public class JVExceptionTool extends JVBoxTool {

    public JVExceptionTool(String text, Image image) {
        super(text, image);
    }

    protected Cursor loadDefaultCursor() {
        // return AwtUtil.loadCursor(OOModule.class,
        // "international/resources/exceptionCursor.gif", new Point(8,8),
    ...
    protected final void createBox(int x, int y) {
        try {
            createClass(JVClassCategory.EXCEPTION, LocaleMgr.action.getString("ExceptionCreation"),
                    x, y);
        } catch (Exception ex) {
