...
class UMLSummaryPage extends AbstractPage {

    private StartupWizardModel model = null;
    private JPanel iconsPanel = new JPanel();

    UMLSummaryPage(StartupWizardModel model) {
    ...
    public void load() throws DbException {
        ...
        if (model.getUmlModelType() == StartupWizardModel.UML_MODEL_TYPE_CLASS
                || model.getUmlModelType() == StartupWizardModel.UML_MODEL_TYPE_PACKAGE) {
            metaclass = DbJVClassModel.metaClass;
        } else {
            metaclass = DbBEModel.metaClass;
        }

        iconsPanel.removeAll();
        StartupWizardUtilities.displayAvailableTools(model, iconsPanel, metaclass);
