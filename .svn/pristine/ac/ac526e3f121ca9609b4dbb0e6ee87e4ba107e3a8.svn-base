...
public class JVClassTool extends JVBoxTool {

    public JVClassTool(String text, Image image) {
        super(text, image);
    }

    protected Cursor loadDefaultCursor() {
        // return AwtUtil.loadCursor(OOModule.class,
        // "international/resources/classCursor.gif", new Point(8,8),
        // "box");//NOT LOCALIZABLE, not yet
        return AwtUtil.createCursor(GraphicUtil.loadImage(JavaModule.class,
                "db/resources/dbjvclass.gif"), new Point(8, 8), "box", true); // NOT
        // LOCALIZABLE,
        // not
        // yet
    }
    ...
    protected final void createBox(int x, int y) {
        try {
            createClass(JVClassCategory.CLASS, LocaleMgr.action.getString("ClassCreation"), x, y);
        } catch (Exception ex) {
