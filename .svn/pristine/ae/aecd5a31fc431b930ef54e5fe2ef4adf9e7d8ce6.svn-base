...
public class OOAggregation extends IntDomain {

    static final long serialVersionUID = 5189196850327543447L;

    public static final int NONE = 1;
    public static final int AGGREGATE = 2;
    public static final int COMPOSITE = 3;

    public static final OOAggregation[] objectPossibleValues = new OOAggregation[] {
            new OOAggregation(NONE), new OOAggregation(AGGREGATE), new OOAggregation(COMPOSITE) };

    public static final String[] stringPossibleValues = new String[] {
            LocaleMgr.misc.getString("None"), LocaleMgr.misc.getString("Aggregate"),
            LocaleMgr.misc.getString("Composite") };

    public static OOAggregation getInstance(int value) {
        return objectPossibleValues[objectPossibleValues[0].indexOf(value)];
    }

    //Parameterless constructor
    public OOAggregation() {
    }

    protected OOAggregation(int value) {
        super(value);
    }

    public final DbtAbstract duplicate() {
        return new OOAggregation(value);
    }
