 ...
 * </p>
 * <p>
 * This dictionary class reads words one per line. Make sure that your word list is formatted in
 * this way (most are).
 * </p>
 */
public class GenericSpellDictionary2 extends SpellDictionaryASpell {
    ...
    /**
     * A field indicating the initial hash map capacity (16KB) for the main dictionary hash map.
     * Interested to see what the performance of a smaller initial capacity is like.
     */
    private final static int INITIAL_CAPACITY = 16 * 1024;

    /**
     * The hashmap that contains the word dictionary. The map is hashed on the doublemeta code. The
     * map entry contains a LinkedList of words that have the same double meta code.
     */
    protected HashMap<String, List<String>> mainDictionary = new HashMap<String, List<String>>(
            INITIAL_CAPACITY);
     ...
     *             when the words list file could not be located on the system.
     * @throws java.io.IOException
     *             when problems occurs while reading the words list file
     */
    public GenericSpellDictionary2(File wordList) throws FileNotFoundException, IOException {
     ...
     *            The file containing dictionary as a words list.
     * @param phonetic
     *            The file containing the phonetic transformation information.
     * @throws java.io.FileNotFoundException
     *             when the words list or phonetic file could not be located on the system
     * @throws java.io.IOException
     *             when problems occurs while reading the words list or phonetic file
     */
    public GenericSpellDictionary2(File wordList, File phonetic) throws FileNotFoundException,
            IOException {
    ...
    public GenericSpellDictionary2(URL url) throws FileNotFoundException, IOException {
        super((File) null);
        dictFile = null; //wordList;
        InputStream is = url.openStream();
        Reader r = new InputStreamReader(is);
        createDictionary(new BufferedReader(r));
        is.close();
    }
     ...
     * <p>
     * This is a very slow function. On my machine it takes quite a while to load the data in. I
     * suspect that we could speed this up quite allot.
     */
    ...
    public List<String> getWords(String code) {
        ...
        List<String> mainDictResult = (List<String>) mainDictionary.get(code);
