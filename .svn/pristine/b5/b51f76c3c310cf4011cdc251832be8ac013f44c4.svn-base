...
public class MultiDbINFDbspaceEditor extends MultiDbSemanticalObjectEditor {
    ...
    protected Collection getSelectionSet(DbObject parentDbo) throws DbException {
        ...
        /*
         * if (parentDbo instanceof DbORDatabase){ int a = 3; return null; } else {
         */
        ...
        if (model != null) {
            ...
            if (database != null) {
                dbEnum = database.getComponents().elements(DbINFDbspace.metaClass);
                while (dbEnum.hasMoreElements())
                    dbspaces.add(dbEnum.nextElement());
                dbEnum.close();
            }
        }
        /** Composite classes of Database or database itself */
        else {
            if (parentDbo instanceof DbORDatabase) {
                dbEnum = parentDbo.getComponents().elements(DbINFDbspace.metaClass);
            } else {
                dbEnum = parentDbo.getCompositeOfType(DbORDatabase.metaClass).getComponents()
                        .elements(DbINFDbspace.metaClass);
            }
            while (dbEnum.hasMoreElements())
                dbspaces.add(dbEnum.nextElement());
            dbEnum.close();
