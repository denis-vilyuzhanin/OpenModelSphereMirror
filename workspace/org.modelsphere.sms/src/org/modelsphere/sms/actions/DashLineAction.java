...
public final class DashLineAction extends AbstractTriStatesAction implements
        SelectionActionListener {

    public DashLineAction() {
        super(LocaleMgr.action.getString("DashLine"), LocaleMgr.action.getImageIcon("DashLine"));
        setEnabled(false);
    }

    protected final void doActionPerformed() {
        performTriState(ApplicationContext.getFocusManager().getSelectedObjects(), LocaleMgr.action
                .getString("DashLineUpdate"));
    }
    ...
    protected final Boolean getObjectValue(Object obj) throws DbException {
        ...
        if (go == null)
            return null;
        MetaField metaField = go.getMetaField("m_dashStyle"); // NOT LOCALIZABLE
        if (metaField != null)
