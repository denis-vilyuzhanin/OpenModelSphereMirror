...
public class MultiDbIBMTablespaceEditor extends MultiDbSemanticalObjectEditor {

    protected DbEnumeration getSelectionEnum(DbObject parentDbo) throws DbException {
        DbEnumeration dbEnum = null;
        DbIBMDataModel model = (DbIBMDataModel) parentDbo
                .getCompositeOfType(DbIBMDataModel.metaClass);
        DbORDatabase database = model.getDeploymentDatabase();
        if ((database != null) && (database instanceof DbIBMDatabase)) {
            DbIBMDatabase db = (DbIBMDatabase) database;
            dbEnum = db.getComponents().elements(DbIBMTablespace.metaClass);
        } // end if
