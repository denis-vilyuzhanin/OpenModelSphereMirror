...
public class MultiDbOROperationLibraryEditor extends MultiDbSemanticalObjectEditor {

    protected Collection getSelectionSet(DbObject parentDbo) throws DbException {
        ArrayList operationLibraries = new ArrayList();
        DbEnumeration dbEnum = parentDbo.getProject().componentTree(DbOROperationLibrary.metaClass);
        while (dbEnum.hasMoreElements()) {
            DbOROperationLibrary dbo = (DbOROperationLibrary) dbEnum.nextElement();
            if (AnyORObject.getTargetSystem(parentDbo) == AnyORObject.getTargetSystem(dbo)) {
                if (parentDbo instanceof DbORDatabase && dbo.getDeploymentDatabase() != null)
                    continue;
                operationLibraries.add(dbo);
            }
        }
        dbEnum.close();
        return operationLibraries;
    }
}
