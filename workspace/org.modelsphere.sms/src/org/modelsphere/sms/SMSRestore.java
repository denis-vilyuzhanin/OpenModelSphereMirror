...
public abstract class SMSRestore {

    public static void main(String[] args) {
        if (args.length != 1) {
            System.out.println(LocaleMgr.message.getString("restoreUsage"));
            System.exit(1);
        }
        String fileName = StringUtil.appendFileExt(args[0], SMSBackup.kFileExt);
        VersionConverter converter = new SMSVersionConverter();
        Application.initMeta();
        try {
            Class dbClass = Class.forName("org.modelsphere.jack.baseDb.db.DbGemStone"); // NOT
            // LOCALIZABLE
            Method method = dbClass.getDeclaredMethod("restoreRepository", // NOT
                    // LOCALIZABLE
            ...
            Boolean done = (Boolean) method.invoke(null, new Object[] {
                    ...
                    new Long(SMSBackup.CHECKWORD), fileName, converter });
            if (done.booleanValue())
                System.out.println(LocaleMgr.message.getString("restoreCompleted"));
        } catch (Exception e) {
        }
        System.exit(0);
    }
