...
final class SelectAllActorsAction extends AbstractSelectAction {

    SelectAllActorsAction() {
        super(BETextUtil.getSingleton().getSelectAllText(DbBEActor.metaClass));
        setVisibilityMode(VISIBILITY_ALWAYS_VISIBLE_IN_TOOLBAR);
        setVisible(false);
        setEnabled(false);
    }

    protected final void doActionPerformed() {
        doActionPerformed(BEActorBox.class);
    }
    ...
    public final void currentFocusChanged(Object oldFocusObject, Object focusObject)
        ...
        if (focusObject instanceof ApplicationDiagram) {
            ...
            if (composite instanceof DbBEUseCase) {
                setName(BETextUtil.getSingleton().getSelectAllText(composite, DbBEActor.metaClass));
                setVisible(true);
                setEnabled(true);
                return;
            }
