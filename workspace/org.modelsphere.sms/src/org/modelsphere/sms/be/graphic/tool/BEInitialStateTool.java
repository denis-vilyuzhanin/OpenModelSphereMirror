...
public final class BEInitialStateTool extends BEUseCaseTool {

    private Cursor cursor;

    public BEInitialStateTool(String text) {
        super(text, GraphicUtil.loadImage(DbSMSStereotype.class, "resources/start-small.gif"));
    ...
    protected Cursor loadDefaultCursor() {
        return AwtUtil.loadCursor(DbSMSStereotype.class, "resources/start-small.gif", new Point(8,
                8), "start", true); // NOT
        // LOCALIZABLE,
        // not
        // yet
    ...
    public String getText(DbObject notation) {
        return LocaleMgr.action.getString("InitialStateCreation");
    }
                ...
                while (enum2.hasMoreElements()) {
                    ...
                    if (stereotype.getStereotypeMetaClass()
                        ...
                        if (stereotype.isBuiltIn())
                            if (stereotype.getName().equals("start")) {
                                useCase.setUmlStereotype(stereotype);
                                useCase.setName("Start");
                            }
                    }
                }
                enum2.close();
