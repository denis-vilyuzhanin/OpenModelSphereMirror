...
public final class BEQualifierUnlinkTool extends LinkUnlinkTool {
    private static final String kTransName = LocaleMgr.misc.getString("UnlinkQualifier");

    public BEQualifierUnlinkTool(String text, Image image) {
        super(0, text, image);
            ...
            if (gc instanceof BEUseCaseBox) {
                ...
                for (int i = 0; i < linkObjs.length; i++) {
                    if (linkObjs[i] instanceof DefaultComparableElement) {
                        ...
                        DbBEUseCaseQualifier assignQualifierFound = null;
                        while (dbEnum.hasMoreElements()) {
                            DbBEUseCaseQualifier assignQual = (DbBEUseCaseQualifier) dbEnum
                                    .nextElement();
                            if (assignQual.getQualifier() == qualifier) {
                                assignQualifierFound = assignQual;
                                break;
                            }
                        }
                        dbEnum.close();
                        if (assignQualifierFound != null)
                            assignQualifierFound.remove();
                    }
                }
                ...
                for (int i = 0; i < linkObjs.length; i++) {
                    if (linkObjs[i] instanceof DefaultComparableElement) {
                        ...
                        DbBEActorQualifier assignQualifierFound = null;
                        while (dbEnum.hasMoreElements()) {
                            DbBEActorQualifier assignQual = (DbBEActorQualifier) dbEnum
                                    .nextElement();
                            if (assignQual.getQualifier() == qualifier) {
                                assignQualifierFound = assignQual;
                                break;
                ...
                for (int i = 0; i < linkObjs.length; i++) {
                    if (linkObjs[i] instanceof DefaultComparableElement) {
                        ...
                        DbBEStoreQualifier assignQualifierFound = null;
                        while (dbEnum.hasMoreElements()) {
                            DbBEStoreQualifier assignQual = (DbBEStoreQualifier) dbEnum
                                    .nextElement();
                            if (assignQual.getQualifier() == qualifier) {
                                assignQualifierFound = assignQual;
                                break;
                ...
                for (int i = 0; i < linkObjs.length; i++) {
                    if (linkObjs[i] instanceof DefaultComparableElement) {
                        ...
                        DbBEFlowQualifier assignQualifierFound = null;
                        while (dbEnum.hasMoreElements()) {
                            DbBEFlowQualifier assignQual = (DbBEFlowQualifier) dbEnum.nextElement();
                            if (assignQual.getQualifier() == qualifier) {
                                assignQualifierFound = assignQual;
                                break;
                            }
