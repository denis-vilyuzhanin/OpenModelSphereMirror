...
final class NavigableAction extends AbstractTriStatesAction implements SelectionActionListener {

    NavigableAction() {
        super(LocaleMgr.action.getString("navigable"));
    }

    protected final void doActionPerformed() {
        performTriState(ApplicationContext.getFocusManager().getSelectedSemanticalObjects(),
                LocaleMgr.action.getString("navigable"));
    }
    ...
    public final void updateSelectionAction() throws DbException {
        ...
        for (int i = 0; i < semObjs.length; i++) {
            ...
            if (assocEnd != null)
                state = updateTriState(state, assocEnd.isNavigable());
        }
        setState(state);
    }
