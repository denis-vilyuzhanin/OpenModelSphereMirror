...
public class AbstractModifierAction extends AbstractTriStatesAction implements
        SelectionActionListener {

    AbstractModifierAction() {
        super(LocaleMgr.action.getString("Abstract"), LocaleMgr.action.getImageIcon("Abstract"));
    ...
    protected final void doActionPerformed() {
        performTriState(ApplicationContext.getFocusManager().getSelectedSemanticalObjects(),
                LocaleMgr.action.getString("AbstractModifierUpdate"));
    }
    ...
    private MetaField getMetaField(DbObject semObj) {
        ...
        if (semObj instanceof DbOOClass)
            field = DbOOClass.fAbstract;
        else if (semObj instanceof DbOOMethod)
            field = DbOOMethod.fAbstract;
        return field;
    }
