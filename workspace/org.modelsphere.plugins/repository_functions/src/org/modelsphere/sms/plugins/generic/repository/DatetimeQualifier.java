...
/**
 * For a domain, gets its datetime qualifier. <br>
 * Target System : <b>All Except Java</b><br>
 * Type : <b>User Function</b><br>
 * Parameters : none.<br>
 */
public class DatetimeQualifier extends UserDefinedField {
    private static final PluginSignature signature = new PluginSignature("DatetimeQualifier",
            "$Revision: 4 $", ApplicationContext.APPLICATION_AUTHOR, "$Date: 2009/04/14 14:00p $",
            212); // NOT LOCALIZABLE

    private static final String typeName = new String("DATETIME"); // NOT LOCALIZABLE

    public DatetimeQualifier() {
    } //Parameter-less constructor required by jack.io.Plugins

    public DatetimeQualifier(String ruleName, String subrule, Modifier[] modifiers)
    ...
    public boolean expand(Writer output, Serializable object, Rule.RuleOptions options)
        ...
        if (!(object instanceof DbORDomain)) {
            return false;
        }

        DbORDomain domain = (DbORDomain) object;

        try {
            if (domain.getName().toUpperCase().indexOf(typeName.toString()) < 0)
                return false; // there is no qualifier
            ...
            int startPosition = typeName.length();
            output.write(domain.getName().substring(startPosition, domain.getName().length())
                    .trim());
            expanded = true;
