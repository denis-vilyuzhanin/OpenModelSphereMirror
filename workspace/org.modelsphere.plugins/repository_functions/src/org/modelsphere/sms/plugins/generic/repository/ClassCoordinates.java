...
/**
 * For a graphical class, gets its XY coordonates. <br>
 * Target System : <b>Java</b><br>
 * Type : <b>User Function</b><br>
 * Returns : a string of the form ''x1,y1,x2,y2'', where :<br>
 * - x1 and y1 represent the graphical class' left upper corner.<br>
 * - x2 and y2 represent the graphical class' right lower corner.<br>
 */
public final class ClassCoordinates extends UserDefinedField {
    private static final PluginSignature signature = new PluginSignature("ClassCoordinates",
            "$Revision: 3 $", ApplicationContext.APPLICATION_AUTHOR, "$Date: 2009/04/14 14:00p $",
            212); // NOT LOCALIZABLE

    public ClassCoordinates() {
    } //Parameter-less constructor required by jack.io.Plugins

    public ClassCoordinates(String ruleName, String subrule, Modifier[] modifiers)
    ...
    public boolean expand(Writer output, Serializable object, Rule.RuleOptions options)
        ...
        DbOOAdtGo classGo = (DbOOAdtGo) object;
        try {
            DbOODiagram diagram = (DbOODiagram) classGo.getComposite();;
            GraphicComponent gc;
            Rectangle rect;
            Point origin;
            ...
            gc = (GraphicComponent) classGo.getGraphicPeer();
            ...
            output.write(rect.x + "," + rect.y + "," + (rect.x + rect.width) + ","
                    + (rect.y + rect.height));
            //output.write(associationEnd.getMultiplicity().getUMLMultiplicityLabel());
            expanded = true;
