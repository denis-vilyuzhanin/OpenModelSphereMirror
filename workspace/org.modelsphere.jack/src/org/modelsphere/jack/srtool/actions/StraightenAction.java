...
public final class StraightenAction extends AbstractApplicationAction implements
        SelectionActionListener {

    public StraightenAction() {
        super(LocaleMgr.action.getString("straighten"), LocaleMgr.action.getImageIcon("straighten"));
        setAccelerator(KeyStroke.getKeyStroke(LocaleMgr.action.getAccelerator("straighten"),
                ActionEvent.CTRL_MASK));
        this.setMnemonic(LocaleMgr.action.getMnemonic("straighten"));
        setEnabled(false);
    ...
    protected final void doActionPerformed() {
        ...
        try {
            DbMultiTrans.beginTrans(Db.WRITE_TRANS, objects, LocaleMgr.action
                    .getString("straighten"));
            for (int i = 0; i < objects.length; i++) {
                DbObject lineGo = ((ActionInformation) objects[i]).getGraphicalObject();
                DbGraphic.createPolyline(lineGo, true);
            }
